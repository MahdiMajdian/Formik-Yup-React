...
const SignUp = () => {
    const validate = Yup.object().shape({
        ...
        confirmPassword: Yup.string().oneOf([Yup.ref('password')], 'passwords must be the same').required('Password field must be filled'),
    })
    return (
        <Formik
            initialValues={{
            ...
            {({ dirty, isValid }) => (
                <div className='p-8 px-16'>
                    ...
                    <Form>
                        ...
                        <button type='submit' disabled={!dirty || !isValid} className={`rounded bg-green-500 hover:bg-green-600 disabled:bg-green-300 mt-4 mr-4 px-4 py-2 text-white font-medium`}>Submit</button>
                        <button type='reset' className={`rounded text-red-600 hover:text-white hover:bg-red-600 mt-4 mr-4 px-4 py-2 border border-red-600 font-medium`}>Reset</button>
                    </Form>
                </div>
            )}
        </Formik>
